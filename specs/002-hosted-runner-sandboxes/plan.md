...
is in the four things the move forces: sandbox size becomes deploy-time rather than per-run, so a ceiling is honoured by staying under it (D4), the
...
|---|---|---|
| **Sandbox size is per-deployment, not per-run** (D4) — a workspace whose ceiling sits between two offered sizes gets the smaller one and is not told, so it can receive less processing power or memory than it asked for. | Processing power and memory come from `instance_type` in the Wrangler container configuration, which is attached to a container binding at deploy time. There is no runtime API to size a sandbox. | Rounding *up* to the smallest sufficient size was the first draft and is worse: it hands an administrator more than the ceiling they set, inverting a setting `specs/001-code-factory-mvp` FR-085 describes as constraining consumption. One binding per distinct workspace configuration would match exactly, but grows without bound and redeploys on every settings change. Failing every workspace that does not match a size exactly was rejected as hostile for no safety gain. |
