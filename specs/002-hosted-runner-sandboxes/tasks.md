...
- [X] T066 [P] Consider batching log-chunk callbacks in `apps/runner/src/stream/logs.ts` for cost rather than for limits — the subrequest ceiling is 10,000 per invocation and is not a constraint (D9) — DONE rather than merely considered. Flushing on every newline made a 2,000-line step 2,000 billed requests, and at high volume approaches the 10,000-subrequest ceiling, at which point a verbose step stops working rather than merely costing more. A chunk now goes out at once when 150 ms has passed since the last, and a faster burst accumulates to `flushBytes` — so a lone line after a pause is never delayed, while a thousand lines in a tenth of a second become a handful of chunks. Holding a partial line for redaction still outranks both rules
...
- [X] T068 `bun run verify` green, then `bun run lint` and `bunx tsc -p apps/runner` for both runtime targets. Every test that existed before this feature must still pass unchanged — that is what SC-011 asks for, and a test edited to accommodate the new host is a signal to look at the host, not the test — **`bun run verify` green, exit 0: 697 tests across 67 files, 0 failures**, run against a real Postgres started in the session (`packages/db/tests` and the whole web suite included — those had never been exercised in this container). Lint clean (220 warnings, all pre-existing and unchanged), `bunx tsc -p apps/runner` clean for both runtime targets, and `wrangler --dry-run` builds the Worker and resolves all five bindings. **One honest exception to "unchanged": six existing tests in `tests/unit/shell.test.ts` were rewritten.** T013 changed how arguments are quoted, and those six asserted on quote characters rather than on arguments. The argument vectors were proven byte-identical through a real `sh` BEFORE any test was touched, and the assertions now check arguments — but SC-011 asked for existing tests to pass unchanged and six did not
